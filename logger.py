...
class Logger(object):
    def __init__(self, logdir):
        ...
        self.prob_file = os.path.join(self.logdir, 'prob.csv')
    ...
    def log_prob(self, probs):
        if self.prob_keys is None:
            self.prob_keys = [k for k in probs.keys()]
            with open(self.prob_file, 'w') as f:
                writer = csv.DictWriter(f, fieldnames=self.prob_keys)
                writer.writeheader()
                writer.writerow(probs)
        else:
            with open(self.prob_file, 'a') as f:
                writer = csv.DictWriter(f, fieldnames=self.prob_keys)
                writer.writerow(probs)
